...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!in_array($fieldType, ['text', 'url', 'textarea'], true)) {
        ...
        try {
            $pdo->prepare(
                'UPDATE profile_fields
                 SET field_name=?, field_label=?, field_type=?, field_icon=?, sort_order=?, is_active=?
                 WHERE id=?'
            )->execute([$fieldName, $fieldLabel, $fieldType, $fieldIcon, $sortOrder, $isActive, $fieldId]);
