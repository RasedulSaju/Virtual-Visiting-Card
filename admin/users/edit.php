            ...
            if ($password !== '') {
                ...
                $pdo->prepare(
                    'UPDATE users SET username=?, email=?, password_hash=?, role=?,
                     can_edit_profile=?, bio=?, profile_image=?, meta_robots=? WHERE id=?'
                )->execute([$username, $email, $hash, $role, $canEdit, $bio, $profileImage, $metaRobots, $userId]);
            } else {
                $pdo->prepare(
                    'UPDATE users SET username=?, email=?, role=?,
                     can_edit_profile=?, bio=?, profile_image=?, meta_robots=? WHERE id=?'
                )->execute([$username, $email, $role, $canEdit, $bio, $profileImage, $metaRobots, $userId]);
            }
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            <label class="form-label fw-semibold">
                                <i class="fas fa-search me-1 text-secondary"></i>Search Visibility
                            </label>
                            ...
                            <div class="form-text">Controls this profile's robots tag &amp; sitemap.</div>
                        </div>
                        <div class="col-12">
                            <div class="form-outline">
