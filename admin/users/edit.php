            ...
            if ($password !== '') {
                ...
                $pdo->prepare(
                    'UPDATE users SET username=?, email=?, password_hash=?, role=?,
                     can_edit_profile=?, bio=?, profile_image=? WHERE id=?'
                )->execute([$username, $email, $hash, $role, $canEdit, $bio, $profileImage, $userId]);
            } else {
                $pdo->prepare(
                    'UPDATE users SET username=?, email=?, role=?,
                     can_edit_profile=?, bio=?, profile_image=? WHERE id=?'
                )->execute([$username, $email, $role, $canEdit, $bio, $profileImage, $userId]);
            }
