...
if (!$adminExists && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$username || !$email || !$password) {
        ...
        try {
            // Re-check for existing admin to guard against a race condition
            // (e.g. two people submitting the form at nearly the same time).
        ...
        <div class="card-body p-4">
            ...
            <?php elseif ($adminExists): ?>
                <div class="alert alert-info text-center">
                    ...
                    <strong>An admin account already exists.</strong>
                </div>
                <div class="warning-banner mb-3">
                    <i class="fas fa-exclamation-triangle text-warning me-1"></i>
                    <strong>Security Warning:</strong> Delete <code>setup.php</code> from your server right now. Leaving it accessible is a critical security risk.
