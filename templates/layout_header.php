<?php
declare(strict_types=1);

// Ensure helpers are loaded (in case template is used directly)
if (!function_exists('getNavPages')) {
    require_once dirname(__DIR__) . '/helpers.php';
}

$_navPages   = getNavPages();
$_pageTitle  = isset($pageTitle) ? $pageTitle . ' — ' . APP_NAME : APP_NAME;
$_activeUser = null;

if (isLoggedIn()) {
    try {
        $s = getDB()->prepare('SELECT id, username, role, profile_image FROM users WHERE id = ? LIMIT 1');
        $s->execute([$_SESSION['user_id']]);
        $_activeUser = $s->fetch();
    } catch (PDOException) {}
}

// Current URL segment for active nav detection
$_currentSlug = trim($_GET['url'] ?? '', '/');
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title><?= e($_pageTitle) ?></title>
<?php
// ── Open Graph ────────────────────────────────────────────────
$_og = array_merge([
    'type'        => 'website',
    'title'       => $_pageTitle,
    'description' => 'Visit ' . APP_NAME,
    'image'       => BASE_URL . 'assets/img/default-avatar.svg',
    'url'         => BASE_URL . ltrim($_currentSlug, '/'),
], $ogData ?? []);
?>
    <meta property="og:type"        content="<?= e($_og['type']) ?>">
    <meta property="og:title"       content="<?= e($_og['title']) ?>">
    <meta property="og:description" content="<?= e($_og['description']) ?>">
    <meta property="og:image"       content="<?= e($_og['image']) ?>">
    <meta property="og:url"         content="<?= e($_og['url']) ?>">
    <meta property="og:site_name"   content="<?= e(APP_NAME) ?>">
    <meta name="twitter:card"       content="summary">
    <meta name="twitter:title"      content="<?= e($_og['title']) ?>">
    <meta name="twitter:description" content="<?= e($_og['description']) ?>">
    <meta name="twitter:image"      content="<?= e($_og['image']) ?>">
    <!-- Fonts -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;600;700&display=swap" rel="stylesheet">
    <!-- Font Awesome 6 -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
    <!-- MDB 5 Free -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/mdb-ui-kit/6.4.0/mdb.min.css">
    <!-- Custom -->
    <link rel="stylesheet" href="<?= BASE_URL ?>assets/css/custom.css">
<?php
// ── Analytics scripts (head) ──────────────────────────────────
$_a_ga4     = getSetting('analytics_ga4_id');
$_a_gtm     = getSetting('analytics_gtm_id');
$_a_clarity = getSetting('analytics_clarity_id');
$_a_fbpx    = getSetting('analytics_fb_pixel_id');
$_a_hotjar  = getSetting('analytics_hotjar_id');
$_a_plaus   = getSetting('analytics_plausible_domain');
$_a_cHead   = getSetting('analytics_custom_head');
$_a_cBody   = getSetting('analytics_custom_body');

// ── Google Tag Manager ────────────────────────────────────────
if ($_a_gtm !== ''): ?>
    <!-- Google Tag Manager -->
    <script>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':
    new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],
    j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src=
    'https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);
    })(window,document,'script','dataLayer','<?= e($_a_gtm) ?>');</script>
    <!-- End Google Tag Manager -->
<?php endif; ?>

<?php // ── Google Analytics 4 ──────────────────────────────────
if ($_a_ga4 !== '' && $_a_gtm === ''): // Skip if GTM is active — add GA4 via GTM instead ?>
    <!-- Google Analytics 4 -->
    <script async src="https://www.googletagmanager.com/gtag/js?id=<?= e($_a_ga4) ?>"></script>
    <script>
        window.dataLayer = window.dataLayer || [];
        function gtag(){dataLayer.push(arguments);}
        gtag('js', new Date());
        gtag('config', '<?= e($_a_ga4) ?>');
    </script>
    <!-- End Google Analytics 4 -->
<?php endif; ?>

<?php // ── Microsoft Clarity ───────────────────────────────────
if ($_a_clarity !== ''): ?>
    <!-- Microsoft Clarity -->
    <script type="text/javascript">
        (function(c,l,a,r,i,t,y){
            c[a]=c[a]||function(){(c[a].q=c[a].q||[]).push(arguments)};
            t=l.createElement(r);t.async=1;t.src="https://www.clarity.ms/tag/"+i;
            y=l.getElementsByTagName(r)[0];y.parentNode.insertBefore(t,y);
        })(window, document, "clarity", "script", "<?= e($_a_clarity) ?>");
    </script>
    <!-- End Microsoft Clarity -->
<?php endif; ?>

<?php // ── Meta (Facebook) Pixel ───────────────────────────────
if ($_a_fbpx !== ''): ?>
    <!-- Meta Pixel -->
    <script>
        !function(f,b,e,v,n,t,s){if(f.fbq)return;n=f.fbq=function(){n.callMethod?
        n.callMethod.apply(n,arguments):n.queue.push(arguments)};if(!f._fbq)f._fbq=n;
        n.push=n;n.loaded=!0;n.version='2.0';n.queue=[];t=b.createElement(e);t.async=!0;
        t.src=v;s=b.getElementsByTagName(e)[0];s.parentNode.insertBefore(t,s)}(window,
        document,'script','https://connect.facebook.net/en_US/fbevents.js');
        fbq('init', '<?= e($_a_fbpx) ?>');
        fbq('track', 'PageView');
    </script>
    <noscript>
        <img height="1" width="1" style="display:none"
             src="https://www.facebook.com/tr?id=<?= e($_a_fbpx) ?>&ev=PageView&noscript=1" alt="">
    </noscript>
    <!-- End Meta Pixel -->
<?php endif; ?>

<?php // ── Hotjar ──────────────────────────────────────────────
if ($_a_hotjar !== ''): ?>
    <!-- Hotjar -->
    <script>
        (function(h,o,t,j,a,r){
            h.hj=h.hj||function(){(h.hj.q=h.hj.q||[]).push(arguments)};
            h._hjSettings={hjid:<?= (int)$_a_hotjar ?>,hjsv:6};
            a=o.getElementsByTagName('head')[0];
            r=o.createElement('script');r.async=1;
            r.src=t+h._hjSettings.hjid+j+h._hjSettings.hjsv;
            a.appendChild(r);
        })(window,document,'https://static.hotjar.com/c/hotjar-','.js?sv=');
    </script>
    <!-- End Hotjar -->
<?php endif; ?>

<?php // ── Plausible Analytics ─────────────────────────────────
if ($_a_plaus !== ''): ?>
    <!-- Plausible Analytics -->
    <script defer data-domain="<?= e($_a_plaus) ?>"
            src="https://plausible.io/js/script.js"></script>
    <!-- End Plausible -->
<?php endif; ?>

<?php // ── Custom Head Code ────────────────────────────────────
if ($_a_cHead !== ''): ?>
    <!-- Custom Head Code -->
    <?= $_a_cHead /* Admin-entered, trusted */ ?>
    <!-- End Custom Head Code -->
<?php endif; ?>
</head>
<body>
<?php // ── GTM noscript (must be first element after <body>) ────
if (!empty($_a_gtm)): ?>
<!-- Google Tag Manager (noscript) -->
<noscript>
    <iframe src="https://www.googletagmanager.com/ns.html?id=<?= e($_a_gtm) ?>"
            height="0" width="0" style="display:none;visibility:hidden"></iframe>
</noscript>
<!-- End Google Tag Manager (noscript) -->
<?php endif; ?>
<?php if (!empty($_a_cBody)): ?>
<!-- Custom Body Code -->
<?= $_a_cBody ?>
<!-- End Custom Body Code -->
<?php endif; ?>

<!-- ── Navbar ──────────────────────────────────────────────── -->
<nav class="navbar navbar-expand-lg cms-navbar sticky-top shadow-sm">
    <div class="container">
        <a class="navbar-brand cms-brand" href="<?= BASE_URL ?>">
            <i class="fas fa-layer-group me-2"></i><?= e(APP_NAME) ?>
        </a>
        <button class="navbar-toggler border-0" type="button"
                data-bs-toggle="collapse" data-bs-target="#navMain"
                aria-controls="navMain" aria-expanded="false">
            <i class="fas fa-bars"></i>
        </button>
        <div class="collapse navbar-collapse" id="navMain">

            <!-- Dynamic page links -->
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                <?php foreach ($_navPages as $_np): ?>
                <li class="nav-item">
                    <a class="nav-link <?= $_currentSlug === $_np['slug'] ? 'active' : '' ?>"
                       href="<?= BASE_URL . e($_np['slug']) ?>">
                        <?= e($_np['title']) ?>
                    </a>
                </li>
                <?php endforeach; ?>
            </ul>

            <!-- Auth section -->
            <ul class="navbar-nav ms-auto align-items-center">
                <?php if ($_activeUser): ?>
                    <?php if ($_activeUser['role'] === 'admin'): ?>
                    <li class="nav-item me-1">
                        <a class="nav-link cms-admin-link" href="<?= BASE_URL ?>admin/">
                            <i class="fas fa-cog me-1"></i>Admin
                        </a>
                    </li>
                    <?php endif; ?>
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle d-flex align-items-center gap-2 py-2"
                           href="#" id="userDrop" role="button"
                           data-bs-toggle="dropdown" aria-expanded="false">
                            <img src="<?= avatarUrl($_activeUser['profile_image']) ?>"
                                 class="rounded-circle cms-nav-avatar" width="32" height="32"
                                 style="object-fit:cover;" alt="avatar">
                            <span class="d-none d-lg-inline"><?= e($_activeUser['username']) ?></span>
                        </a>
                        <ul class="dropdown-menu dropdown-menu-end shadow border-0">
                            <li class="px-3 py-1">
                                <small class="text-muted">Signed in as <strong><?= e($_activeUser['username']) ?></strong></small>
                            </li>
                            <li><hr class="dropdown-divider my-1"></li>
                            <li>
                                <a class="dropdown-item" href="<?= BASE_URL . e($_activeUser['username']) ?>">
                                    <i class="fas fa-id-card me-2 text-muted"></i>My Profile
                                </a>
                            </li>
                            <li>
                                <a class="dropdown-item" href="<?= BASE_URL ?>edit-profile">
                                    <i class="fas fa-pen me-2 text-muted"></i>Edit Profile
                                </a>
                            </li>
                            <li><hr class="dropdown-divider my-1"></li>
                            <li>
                                <a class="dropdown-item text-danger" href="<?= BASE_URL ?>logout">
                                    <i class="fas fa-sign-out-alt me-2"></i>Sign Out
                                </a>
                            </li>
                        </ul>
                    </li>
                <?php else: ?>
                    <li class="nav-item">
                        <a class="nav-link" href="<?= BASE_URL ?>login">
                            <i class="fas fa-sign-in-alt me-1"></i>Login
                        </a>
                    </li>
                    <?php if (getSetting('registration_open', '1') === '1'): ?>
                    <li class="nav-item ms-1">
                        <a class="btn btn-primary btn-sm px-3" href="<?= BASE_URL ?>register">
                            Sign Up
                        </a>
                    </li>
                    <?php endif; ?>
                <?php endif; ?>
            </ul>
        </div>
    </div>
</nav>

<!-- ── Main Content ───────────────────────────────────────────── -->
<main class="container py-4">
